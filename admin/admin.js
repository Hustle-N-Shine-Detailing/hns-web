(() => {
  ...
  function clear(){userId=null;rows=[];events=[];offset=0;$('requests').replaceChildren();$('traffic-sources').replaceChildren();$('traffic-devices').replaceChildren();$('dashboard').hidden=true;$('logout').hidden=true;$('login-panel').hidden=false;$('password').value='';}
  ...
  async function loadTraffic(){
    ...
    try{const {data,error}=await client.from('site_events').select('event_name,occurred_at,visitor_hash,referrer_host,device_type,utm_source').gte('occurred_at',since).order('occurred_at',{ascending:false}).limit(5000);if(error)throw error;if(!userId)return;events=data||[];showTraffic();}
