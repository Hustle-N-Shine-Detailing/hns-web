(() => {
  ...
  style.textContent = `
    ...
    .campaign-card{display:grid;gap:10px}.campaign-card code{display:block;white-space:normal;overflow-wrap:anywhere;background:#0b0d0f;border:1px solid var(--line);border-radius:10px;padding:10px;font-size:.76rem;color:#dce2e7}
    .campaign-actions{display:flex;gap:8px;flex-wrap:wrap}.campaign-note{color:var(--muted);font-size:.8rem}
  ...
  view.innerHTML = `
    <article class="panel">
      <div class="panel-head"><div><h3>Track every marketing source</h3><small>Use these links instead of the plain homepage URL. Website bookings will keep the source and campaign attached to the lead.</small></div></div>
  ...
  links.forEach(item => {
    ...
    const copy = document.createElement('button'); copy.className = 'btn primary mini'; copy.type = 'button'; copy.textContent = 'Copy tracked link';
    copy.addEventListener('click', async () => {
      try {
        await navigator.clipboard.writeText(url);
        copy.textContent = 'Copied';
        setTimeout(() => { copy.textContent = 'Copy tracked link'; }, 1400);
      } catch {
  ...
  showView = function(name) {
    ...
    if (name === 'campaigns') els.pageTitle.textContent = 'Campaign Links';
  };
})();